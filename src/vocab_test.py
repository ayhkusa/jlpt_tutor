...
<body>
  ...
  <main class=\"app\">
    ...
    <section class=\"stats\">
      ...
      <div class=\"actions\">
        <button data-vowel="a" type="button">a</button>
        <button data-vowel="i" type="button">i</button>
        <button data-vowel="u" type="button">u</button>
        <button data-vowel="e" type="button">e</button>
        <button data-vowel="o" type="button">o</button>
  ...
  <script>
    ...
    const vowelButtons = Array.from(document.querySelectorAll('.actions button[data-vowel]'));
    ...
    function setActiveVowel(vowel) {{
      ...
      vowelButtons.forEach((button) => {{
        const isActive = button.dataset.vowel === activeVowel;
        button.setAttribute("aria-pressed", isActive ? "true" : "false");
      }});
    }}

    function getVowelGroup(romaji) {{
      for (let i = romaji.length - 1; i >= 0; i -= 1) {{
        const char = romaji[i].toLowerCase();
        if (VOWEL_GROUPS.includes(char)) {{
          return char;
        }}
      }}
      return null;
    ...
    function filterByVowelGroup(items) {{
      if (!activeVowel) {{
        return items;
      }}
      return items.filter((item) => getVowelGroup(item.romaji) === activeVowel);
    }}
    ...
    function render() {{
      ...
      updateStatus(matchingRomaji
        ? `${{activeVowel ? `Romaji-to-kana mode (${{activeVowel}}-group)` : "Romaji-to-kana mode"}}: click any romaji card to toggle known and reveal hiragana.`
        : `${{activeVowel ? `Hiragana-to-romaji mode (${{activeVowel}}-group)` : "Hiragana-to-romaji mode"}}: click any kana card to toggle known and reveal romaji.`);
    }}
    ...
    vowelButtons.forEach((button) => {{
      button.addEventListener("click", () => {{
        const selectedVowel = button.dataset.vowel;
        setActiveVowel(activeVowel === selectedVowel ? null : selectedVowel);
        render();
      }});
    }});
